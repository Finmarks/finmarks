...
if (!existsSync(REPO_DIST)) {
  console.error('bharatbrands: repo dist/ not found — run `pnpm generate` at the repo root first.');
  process.exit(1);
}
...
for (const file of FILES) {
  ...
  if (!existsSync(src)) {
    console.error(`bharatbrands: dist/${file} missing — run \`pnpm generate\` at the repo root.`);
    process.exit(1);
  }
  await copyFile(src, join(DATA, file));
}

console.log(`bharatbrands: copied ${FILES.length} data files into packages/bharatbrands/data/`);
